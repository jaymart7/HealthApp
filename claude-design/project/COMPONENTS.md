...
| NumericStepperField | Reusable | Confirm, and every entry form | `label, value, unitSuffix, onIncrement, onDecrement, error?` | Value + unit + circular +/− buttons, tabular-nums. |
| RulerPickerField `NEW` | Reusable | Basics (age, height, weight, target weight) | `label, value: Double?, range, step, unit, onValueChange, decimals?, trailing?` | Big tabular value over a draggable 44dp tick scale with a fixed `primary` caret — the scale translates, the caret does not. Drag 1:1, fling with decay, snap on settle, 12dp rubber-band at the ends. Tapping the value opens the numeric IME and collapses the scale to a range line plus Done. Null value = unset ("—", hint, `outlineVariant` ticks); first touch sets the midpoint. Tick geometry is dp and does not scale with the font. One merged node with `ProgressBarRangeInfo` + `setProgress` (TalkBack swipe-to-adjust) plus one-unit increase/decrease custom actions. |
