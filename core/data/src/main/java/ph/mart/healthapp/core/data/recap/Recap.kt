 ...
 *
 * A domain that owns no table, beside `insight/` and `streak/`. It was feature-local while Progress
 * was the only screen that drew it; the coach's report card is the second, and `:feature:*` modules
 * never import each other — `MealType.labelRes`' move for its reason. What stayed behind in
 * `:feature:progress` is `RecapPeriod`, which is three string resources and a coach question: the
 * *window* is a feature's way of naming a span, and [recap] only ever needed the number of days in
 * it.
 */
...
/**
 * [daysLogged] uses the four-domain definition (food, water, weigh-in, or exercise) so this can
 * never disagree with the streak about what a logged day is. [NutritionAverages.daysLogged] inside
 * [averages] is the narrower food-only count, and the two can differ — the caller says so rather
 * than quietly reporting an average over a different denominator.
 *
 * [days] is the window this was folded over, not a label: the Progress tab's `RecapPeriod` and the
 * coach's report card each name it in their own words.
 *
 * Everything from [startWeightKg] down is drawn by the recap *screen* and the coach's card only;
 * the card on the Progress tab renders the six fields above them and nothing else.
 */
data class Recap(
    val days: Int,
    val daysLogged: Int,
 ...
 * months ago. It stays a seven-day figure on every window — [Recap.weightArcKg] is the window's own
 * answer, and the caller labels which is which.
 */
fun recap(
    days: Int,
