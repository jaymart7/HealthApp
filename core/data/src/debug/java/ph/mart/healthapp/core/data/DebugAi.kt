...
import ph.mart.healthapp.core.data.fake.FakeRecipeParseRepository
 ...
 *
 * All ten call sites at once rather than ten switches, because the reason to flip is always
 * the same — *checking a change against the real thing before a release* — and ten booleans is
 * ten ways to leave one on by accident. Splitting it per feature is one line if that ever stops
 * being true.
 *
 * A debug-only escape hatch in the shape `FORCE_ONBOARDING` already uses in `AppRoot.kt`, except
 * that this one lives in a source set rather than behind `BuildConfig.DEBUG`: ten fake
...
internal fun debugRecipeParse(): RecipeParseRepository? = ifFaking { FakeRecipeParseRepository() }
