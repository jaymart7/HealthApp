...
/** UI-only: the range this page's chart is showing, and nothing else. The ticking is Home's row and the authoring is Profile's list. One key out of
 * `ProgressScreenState.ranges`, the way `SleepState` is. */
internal class SupplementsState(range: ChartRange = DEFAULT_CHART_RANGE) {
    ...
    companion object {
        ...
        fun Saver(): Saver<SupplementsState, Any> = listSaver(
            ...
            restore = { saved -> SupplementsState(ChartRange.valueOf(saved[0] as String)) },
        )
    }
}
