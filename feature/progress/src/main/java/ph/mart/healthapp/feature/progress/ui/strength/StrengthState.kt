...
@Composable
internal fun rememberStrengthState(): StrengthState =
    rememberSaveable(saver = StrengthState.Saver()) { StrengthState() }

/** UI-only: the range this page's chart is showing, and nothing else. The workouts are logged in `:feature:training`. One key out of
 * `ProgressScreenState.ranges`, the way `SleepState` is. */
internal class StrengthState(range: ChartRange = DEFAULT_CHART_RANGE) {
