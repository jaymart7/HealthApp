...
/**
 * "3 lifts / 9 sets" — volume, and nothing else. No load, because a routine carries none: it
 * opens at whatever was last lifted.
 *
 * **The weekdays used to be on this line too**, which crammed two different kinds of fact into
 * one grey caption and then said them twice, since the picker under the row already showed them.
 * The schedule lives in the plan zone now; this is figures.
 */
@Composable
internal fun Routine.figures(): List<Figure> = listOf(
    Figure(lifts.size.toString(), pluralStringResource(R.plurals.profile_routine_lifts_unit, lifts.size)),
    Figure(totalSets().toString(), pluralStringResource(R.plurals.profile_routine_sets_unit, totalSets())),
)
