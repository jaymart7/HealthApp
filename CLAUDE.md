...
- **A feature holding more than one flow nests one level per flow.** A single-flow
  ...
  **every subject page is becoming a route**, one commit at a time — a converted one holds the full
  ...
  (`photo`, `sleep`, `mood`, `heart`, `supplement`, `strength`, `fasting`, `activity`, `cycle`, `pressure`, `measurement`, `weight`, `nutrition` so far), an unconverted one still holds a `components/*Detail.kt` body that
